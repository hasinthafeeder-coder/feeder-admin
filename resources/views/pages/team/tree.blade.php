                ...
                <div class="card bg-white border border-white rounded-10 mb-4">
                    <div class="card-body p-20">
                        ...
                        <div class="team-tree-search-wrapper mb-3">
                            <label for="teamTreeSearchInput" class="form-label mb-2 fw-medium">Search User</label>
                            <div class="position-relative">
                                <input id="teamTreeSearchInput" type="text" class="form-control"
                                    placeholder="Search by user ID, name, company, email or phone..." autocomplete="off" />
                        ...
                        <div id="teamTreeViewport" class="team-tree-viewport">
                            ...
                            <div id="teamTreeEmpty" class="alert alert-info d-none mb-0">
                                No referral hierarchy is available yet.
                            </div>
...
@push('styles')
    <style>
        ...
        .team-tree-badge-reseller {
            color: #166534;
            background: rgba(22, 163, 74, 0.12);
        }

        .team-tree-badge-supplier {
            color: #1d4ed8;
            background: rgba(37, 99, 235, 0.12);
        }

        .team-tree-badge-master {
            color: #b45309;
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function normalizePortalBadge(node) {
                if (node.is_master_reseller) {
                    return {
                        label: 'Master',
                        cssClass: 'team-tree-badge-master',
                    };
                }

                if (node.portal_code === 'RESELLER') {
                    return {
                        label: 'Reseller',
                        cssClass: 'team-tree-badge-reseller',
                    };
                }

                if (node.portal_code === 'SUPPLIER') {
                    return {
                        label: 'Supplier',
                        cssClass: 'team-tree-badge-supplier',
                    };
                }
            ...
            function createNodeBranch(node) {
                ...
                const badgeData = normalizePortalBadge(node);
                const badgeMarkup = badgeData ?
                    `<span class="team-tree-badge ${badgeData.cssClass}">${badgeData.label}</span>` : '';
            ...
            function showChildrenEmpty(nodeState) {
                nodeState.childrenWrapEl.innerHTML =
                    '<div class="team-tree-children-empty">No direct referrals</div>';
            }
            ...
            async function expandNodeById(userId) {
                ...
                if (!nodeState || !nodeState.data.has_children) {
                    return;
                }
                ...
                if (!nodeState.childrenLoaded) {
                    ...
                    try {
                        const data = await fetchJson(
                            buildUrl(endpoints.childrenTemplate, nodeState.data.user_uuid),
                        );
            ...
            function getNodeElementById(userId) {
                return state.nodes.get(userId)?.nodeEl || null;
            }
            ...
            async function expandPathToUser(userUuid) {
                ...
                try {
                    ...
                    if (!path.length) {
                        showMessage('No referral path was found for the selected user.');
                        return;
                    }
                    ...
                    for (let index = 0; index < path.length - 1; index++) {
                        ...
                        await expandNodeById(parent.user_id);
                    }

                    const targetNode = getNodeElementById(data.selected_user_id);
                    if (!targetNode) {
            ...
            rootEl.addEventListener('click', async function(event) {
                ...
                try {
                    ...
                    showMessage(error.message || 'Could not load referrals for this user.');
                }
            });
